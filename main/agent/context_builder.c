...
esp_err_t context_build_system_prompt(char *buf, size_t size)
{
    ...
    off += snprintf(buf + off, size - off,
        ...
        "- camera_send_photo: Capture a JPEG photo from the XIAO ESP32S3 Sense camera and stream it directly to Telegram without saving it on ESP32 flash.\n"
        ...
        "- water_pump_control: Turn the water pump relay on GPIO44 on or off. Defaults to active-low relay logic: ON writes LOW, OFF writes HIGH.\n"
        ...
        "- adc_read: Read an analog value from an ADC-capable GPIO. Use for analog sensors such as soil moisture sensors. Optional dry_raw and wet_raw return a calibrated moisture percentage.\n\n"
        "- soil_moisture_report: Read the soil moisture sensor on GPIO7, explain the current plant moisture level, and optionally send the report to Telegram.\n\n"
        ...
        "For analog sensor values, use adc_read on ADC-capable pins; prefer GPIO1-GPIO10 on ESP32-S3 when WiFi is active. "
        "For the plant soil moisture sensor connected to GPIO7, use soil_moisture_report.\n\n"
        "## Water Pump\n"
        "Use water_pump_control for the pump relay on GPIO44 instead of gpio_write. Most relay modules are active-low, so the tool treats ON as GPIO LOW and OFF as GPIO HIGH by default.\n\n"
