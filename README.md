...
You send a message on Telegram. The ESP32-S3 picks it up over WiFi, feeds it into an agent loop — the LLM thinks, calls tools, reads memory — and sends the reply back. Supports **Anthropic (Claude)**, **OpenAI (GPT)**, and **OpenRouter** as providers, switchable at runtime. Everything runs on a single $5 chip with all your data stored locally on flash.
...
mimi> set_api_key sk-ant-api03-... # change API key (Anthropic, OpenAI, or OpenRouter)
mimi> set_model_provider openrouter # switch provider (anthropic|openai|openrouter)
